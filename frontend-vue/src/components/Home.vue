    ...
    methods: {
        async tampilPenjualan() {
            const response = await axios.get(
                `https://api-group9-prognet.manpits.xyz/api/penjualan`
            );
        ...
        async tampilUser() {
            const response = await axios.get(
                `https://api-group9-prognet.manpits.xyz/api/user`
            );
        ...
        async tampilBarang() {
            const response = await axios.get(
                `https://api-group9-prognet.manpits.xyz/api/barang`
            );
        ...
        async tampilSatuan() {
            const response = await axios.get(
                `https://api-group9-prognet.manpits.xyz/api/satuanbarang`
            );
            this.daftarSatuan = response.data;
            //console.log(this.daftarSatuan);
        },
        getSatuan(satuan_id) {
            const satuan = this.daftarSatuan.find(
                (satuan) => satuan.id == satuan_id
            );
        ...
        filterTransaksi() {
            ...
            const filterIdTransaksi =
                this.filterValues.idTransaksi.toLowerCase();

            // Filter daftar penjualan berdasarkan nilai filter
            let filteredTransaksi = this.originalDaftarPenjualan.filter(
                (penjualan) => {
                    const idMatch = penjualan.nomortransaksi
                        .toLowerCase()
                        .includes(filterIdTransaksi);
        ...
        filterKasir() {
            ...
            // Filter daftar penjualan berdasarkan nilai filter
            let filteredKasir = this.originalDaftarPenjualan.filter(
                (penjualan) => {
                    const idMatch =
                        this.getCashierName(penjualan.user_id)
                            .toLowerCase()
                            .includes(filterIdKasir) ||
                        this.getCashierEmail(penjualan.user_id)
                            .toLowerCase()
                            .includes(filterIdKasir);
                    // You can add more filter conditions if needed
                    return idMatch;
        ...
        filterBarang() {
            ...
            let filteredBarang = this.originalDaftarBarang.filter((barang) => {
                const idMatch = barang.kode
                    .toLowerCase()
                    .includes(filterIdBarang);
                // You can add more filter conditions if needed
                return idMatch;
            });
        ...
        filterSatuan() {
            ...
            let filteredBarang = this.originalDaftarBarang.filter((barang) => {
                const idSatuanMatch = this.getSatuan(barang.satuan_id)
                    .toLowerCase()
                    .includes(filterIdSatuan);
                // You can add more filter conditions if needed
            ...
            let filteredSatuan = this.originalDaftarSatuan.filter((satuan) => {
                const idMatch = this.getSatuan(satuan.id)
                    .toLowerCase()
                    .includes(filterIdSatuan);
                // You can add more filter conditions if needed
                return idMatch;
        ...
        filterStok() {
            ...
            let filteredStok = this.originalDaftarBarang.filter((barang) => {
                const stokMatch =
                    barang.stok >= parseInt(filterRangeStok.split("-")[0]) &&
                    barang.stok <= parseInt(filterRangeStok.split("-")[1]);
                // You can add more filter conditions if needed
                return stokMatch;
            });
        ...
        filterHarga() {
            ...
            let filteredHarga = this.originalDaftarBarang.filter((barang) => {
                const hargaMatch =
                    barang.harga >= parseInt(filterRangeHarga.split("-")[0]) &&
                    barang.harga <= parseInt(filterRangeHarga.split("-")[1]);
                // You can add more filter conditions if needed
                return hargaMatch;
            });
...
<template>
        ...
        >
            ...
            <div class="relative">
                ...
                <input
                    ...
                    placeholder="Search by Transaksi"
                    v-model="filterValues.idTransaksi"
                    @input="filterTransaksi"
                />
            </div>
            <div>
                <!-- Filter berdasarkan user dan Gmail -->
                <label for="filter-user-gmail" class="sr-only"
                    >Search Kasir</label
                >
                <div class="relative">
                    <div
                    ...
                    <input
                        type="text"
                        id="filter-user-gmail"
                        class="block pt-2 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg w-80 bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        placeholder="Search by Kasir"
                        v-model="filterValues.idKasir"
                        @input="filterKasir"
                    />
                </div>
            </div>
            <div>
                ...
                <label for="filter-total-harga" class="sr-only"
                    >Filter Total Harga</label
                >
                <div class="relative">
                    <div
                        class="absolute inset-y-0 rtl:inset-r-0 start-0 flex items-center ps-3 pointer-events-none"
                    ...
                    <select
                        id="filter-total-harga"
                        class="block pt-2 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg w-80 bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        @change="filterHargaTransaksi"
                        v-model="filterValues.sortHargaTransaksi"
                    >
                        <option value="">
                            Select by Harga Transaksi Range
                        </option>
                        <option value="besar-ke-kecil">
                            Filter by Harga Terbesar
                        </option>
                        <option value="kecil-ke-besar">
                            Filter by Harga Terkecil
                        </option>
                    </select>
                </div>
            </div>
            <div>
                ...
                <input
                    type="date"
                    id="filter-date"
                    class="text-sm text-gray-900 border border-gray-300 rounded-lg w-36 bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
        ...
        >
            ...
            >
                <tr>
                    ...
                    <th scope="col" class="px-6 py-3">Nama Kasir</th>
            ...
            <tbody>
                <tr
                    v-for="(penjualan, key) in daftarPenjualan"
                ...
                >
                    ...
                    <td class="p-4">
                        <router-link
                            :to="'/penjualan/detail/' + penjualan.id"
                            class="w-full"
                        >
                            <span
                                class="w-full hover:bg-blue-200 bg-blue-100 text-blue-800 text-md font-medium inline-flex gap-2 items-center justify-center px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-blue-400 border border-blue-400"
                            >
                    ...
                    <td class="px-6 py-4">
                        <div class="flex items-center">
                            <svg
                                class="w-5 h-5 text-green-500 dark:text-white mr-3"
                                aria-hidden="true"
                                xmlns="http://www.w3.org/2000/svg"
                                fill="none"
                                viewBox="0 0 20 16"
                    ...
                    <td class="px-6 py-4">
                        {{
                            new Date(penjualan.created_at).toLocaleString(
                                "id-ID"
                            )
                        }}
                    </td>
    ...
    <div class="relative pt-12">
        ...
        >
            <div class="mb-2">
                <!-- Filter kode barang -->
                <label for="filter-kode-barang" class="sr-only"
                    >Filter ID Barang</label
                >
                <div class="relative">
                    <div
                        class="absolute inset-y-0 rtl:inset-r-0 start-0 flex items-center ps-3 pointer-events-none"
                    ...
                    <input
                        type="text"
                        id="filter-kode-barang"
                        class="block pt-2 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg w-80 bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        placeholder="Search by ID Barang"
                        v-model="filterValues.kodeBarang"
                        @input="filterBarang"
                    />
                </div>
            </div>
            <div class="mb-2">
                ...
                <label for="filter-range-harga" class="sr-only"
                    >Filter Range Harga Barang</label
                >
                <div class="relative">
                    <div
                        class="absolute inset-y-0 rtl:inset-r-0 start-0 flex items-center ps-3 pointer-events-none"
                    ...
                    <select
                        id="filter-range-harga"
                        class="block pt-2 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg w-80 bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        @change="filterHarga"
                        v-model="filterValues.rangeHarga"
                    >
                        <option value="">Select Harga Range</option>
                        <option value="1-100000">
                            Filter by range harga Rp. 1.000 - Rp. 100.000
                        </option>
                        <option value="100001-500000">
                            Filter by range harga Rp. 100.001 - Rp. 500.000
                        </option>
                        <option value="500001-1000000">
                            Filter by range harga Rp. 500.001 - Rp. 1.000.000
                        </option>
                        <option value="1000001-5000000">
                            Filter by range harga Rp. 1.000.001 - Rp. 5.000.000
                        </option>
                        <option value="5000001-10000000">
                            Filter by range harga Rp. 5.000.001 - Rp. 10.000.000
                        </option>
                        <option value="10000001-50000000">
                            Filter by range harga Rp. 10.000.001 - Rp.
                            50.000.000
                        </option>
                        <option value="50000001-100000000">
                            Filter by range harga Rp. 50.000.001 - Rp.
                            100.000.000
                        </option>
                        <option value="100000001-500000000">
                            Filter by range harga Rp. 100.000.001 - Rp.
                            500.000.000
                        </option>
                        <option value="500000001-100000000000">
                            Filter by range harga Rp. 500.000.001 - Rp.
                            100.000.000.000
                        </option>
                    </select>
                </div>
            ...
            <div class="mb-2">
                <!-- Filter range stok -->
                <label for="filter-range-stok" class="sr-only"
                    >Filter Range Stok</label
                >
                <div class="relative">
                    <div
                        class="absolute inset-y-0 rtl:inset-r-0 start-0 flex items-center ps-3 pointer-events-none"
                    ...
                    <select
                        id="filter-range-stok"
                        class="block pt-2 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg w-80 bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        @change="filterStok"
                        v-model="filterValues.rangeStok"
                    >
                        ...
                        <option value="101-200">
                            Filter by stok 101 - 200
                        </option>
                        <option value="201-500">
                            Filter by stok 201 - 500
                        </option>
                        <option value="501-1000">
                            Filter by stok 501 - 1000
                        </option>
                        <option value="1001-2000">
                            Filter by stok 1001 - 2000
                        </option>
                    </select>
                </div>
            </div>
            <div class="mb-2">
                ...
                <div class="relative">
                    ...
                    <input
                        ...
                        id="filter-satuan"
                        class="block pt-2 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg w-80 bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        placeholder="Search by Satuan"
                        v-model="filterValues.satuan"
                        @input="filterSatuan"
                    />
                </div>
            </div>
        ...
        >
            ...
            <tbody>
                <tr
                    v-for="(barang, key) in daftarBarang"
                    :key="barang.id"
                    class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600"
                >
                    <td class="px-6 py-3">
                    ...
                    <td class="px-6 py-3">
                        <span
                            class="hover:bg-blue-200 bg-blue-100 text-blue-800 text-md font-medium inline-flex gap-2 items-center justify-center px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-blue-400 border border-blue-400"
                        >
                            {{ barang.kode }}
                        </span>
                    </td>
                    ...
                    <td class="px-6 py-3">
                        <span
                            class="hover:bg-blue-200 bg-blue-100 text-blue-800 text-md font-medium inline-flex gap-2 items-center justify-center px-3 py-1 rounded dark:bg-gray-700 dark:text-blue-400 border border-blue-400"
                        >
